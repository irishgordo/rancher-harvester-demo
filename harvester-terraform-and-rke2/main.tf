...
resource "harvester_volume" "myvol" {
  name      = "myvol"
  namespace = "default"
  size      = "63Gi"
  timeouts {
...
#  harvester_network.mgmt-vlan-minio
resource "harvester_virtualmachine" "minio-vm" {
  depends_on = [
    harvester_cloudinit_secret.cloud-config-minio, harvester_image.minio-img
  ]
